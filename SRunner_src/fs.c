...
int fs_open( char argv[], char* mode, int *file_id )
{
	...
	if ( fid == NULL )
	{
		ufprintf( stderr, "Error: Could not open file: \"%s\"!\n", argv );
		return CB_FS_OPEN_FILE_ERROR;
	}
	else
	{
		*file_id = (int) fid;
		ufprintf( stdout, "Info: Programming file: \"%s\" opened.\n", argv );
		return CB_OK;
	}
}
...
int	fs_close( int file_id )
{
	...
	if ( status )
	{
		ufprintf( stderr, "Error: Could not close file!\n");
...
int fs_size( int file_id, long int *size )
{
	...
	if ( status )
	{
		ufprintf( stderr, "Error: End of file could not be located!" );
	...
	else
	{
		*size = ftell( (FILE*) file_id );
		...
		ufprintf( stdout, "Info: File size: %ld bytes.\n", *size );
		return CB_OK;
	}
}
...
int fs_read( int file_id, int *data )
{
	...
	if ( status == EOF )
	{
		ufprintf( stderr, "Error: Could not read data from file!" );
